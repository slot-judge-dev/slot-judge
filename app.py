...
with st.sidebar:
    ...
    st.divider()
    st.caption("ZERO-PROTOCOL v7.5.3 (Hokuto Exclusive)")
...
if st.button("軍師の最終判断を仰ぐ (Execute Analysis)"):
    ...
    # 内部演算プロトコル
    is_mode_b_confirmed = (shutter_hint == "特定区間で発生(1~64, 129~192等) ※B以上")
    max_ceiling = 896 if is_mode_b_confirmed else (1280 if is_reset else 1536)
    ...
    # モードB以上濃厚時の積極型全ツッパロジック
    elif is_mode_b_confirmed and current_abe >= 150:
        ev = calc_ev(1500 if current_abe > 400 else 800)
        tsuranuki_tag = " | 💎 つらぬき期待度:高" if is_tsuranuki_chance else ""
        st.success(f"🔥 【判定：特攻GO】通常B(896天井)濃厚！256ゾーン経由で全ツッパ 💰期待値: ￥{ev}前後{tsuranuki_tag} | 天井まで残り{max_ceiling - current_abe}あべし")
        ...
        elif current_abe < 150 and is_mode_b_confirmed:
            next_target = "150あべし (通常B濃厚 特攻ライン)"
...
st.caption("© 2026 Protocol ZERO | Daisuke Custom Build v7.5.3 (Hokuto Exclusive) Professional Ultimate")
